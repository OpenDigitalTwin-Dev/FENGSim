    ...
    if (Selectable == true)
    {
        if (SelectDomain == true)
        {
            ...
            if (SelectedVtkActor != NULL)
            {
                ...
                //(*prims)[n]->ShowProperties(Properties);
        ...
        else if (SelectBnd == true)
        {
            ...
            if (SelectedVtkActor != NULL)
            {
                ...
                if ((*bnds)[n]->Type() == Dirichlet)
                    cad_dw->ui->comboBox_3->setCurrentIndex(0);
                else
                    cad_dw->ui->comboBox_3->setCurrentIndex(1);
                cad_dw->ui->doubleSpinBox_7->setValue((*bnds)[n]->GetValue()[0]);
                cad_dw->ui->doubleSpinBox_8->setValue((*bnds)[n]->GetValue()[1]);
                cad_dw->ui->doubleSpinBox_9->setValue((*bnds)[n]->GetValue()[2]);
            }
            else
            {
                cad_dw->ui->comboBox_3->setCurrentIndex(0);
                cad_dw->ui->doubleSpinBox_7->setValue(0);
                cad_dw->ui->doubleSpinBox_8->setValue(0);
                cad_dw->ui->doubleSpinBox_9->setValue(0);
            }
        }
    }
}
...
void VTKWidget::UpdateBndValue()
{
    if (SelectedVtkActor != NULL)
    {
        ...
        if (cad_dw->ui->comboBox_3->currentIndex() == 0)
            (*bnds)[n]->SetType(Dirichlet);
        else if (cad_dw->ui->comboBox_3->currentIndex() == 1)
            (*bnds)[n]->SetType(Neumann);
        double t[3] = {cad_dw->ui->doubleSpinBox_7->value(),
                       cad_dw->ui->doubleSpinBox_8->value(),
                       cad_dw->ui->doubleSpinBox_9->value()};
        (*bnds)[n]->SetValue(t);
    }
}
...
void VTKWidget::MeasurePlotCAD (TopoDS_Shape S, bool t)
{
    ...
    meas_cad_actor->SetVisibility(true);
