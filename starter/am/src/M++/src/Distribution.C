...
// $Header: /public/M++/src/Distribution.C,v 1.10 2009-12-07 08:31:39 wieners Exp $
...
void FullCellsOverlap (Mesh& M) {
    ...
    pout << "Faces Size " << M.Faces::size() << endl;
}
...
class CellBoundaryFaces {
    ...
    short N;
 public:
    CellBoundaryFaces (const Mesh& M, const cell& c) : N(0) {
	for (int i=0; i<c.Faces(); ++i) {
...
inline Buffer& operator << (Buffer& b, const CellBoundaryFaces& bf) { 
    b << bf.size();
    for (int i=0; i<bf.size(); ++i) b << bf[i];
    return b;
}

void FullBndCellsOverlap (Mesh& M) {
    ExchangeBuffer E;
    for (cell c=M.cells(); c!=M.cells_end(); ++c) {
	CellBoundaryFaces bf(M,c);
	...
	while (E.Receive(q).size()<E.Receive(q).Size()) {
	    ...
	    M.InsertOverlapCell(C);
	    for (int i=0; i<C->Faces(); ++i)
		M.Faces::Insert(C->Face(i),C->Center());
	    M.ProcSets::Add(C->Center(),q);
	    for (int i=0; i<C->Corners(); ++i)
		M.ProcSets::Add(C->Corner(i),q);
	    for (int i=0; i<C->Edges(); ++i)
	    ...
	    E.Receive(q) >> n;
	    for (int i=0; i<n; ++i)
		E.Receive(q) >> M.BoundaryFaces::ref();
	}
    }
}
