// file: Solution.C
// author: Jiping Xin

#include "AMProblem.h"

void AMBVP::SetDomain (Mesh& M) {
    int id;
    ReadConfig(Settings, "EXAMPLE", id);
    for (cell c = M.cells(); c != M.cells_end(); c++) {
	Point p(c());
	hash_map<Point,Cell*,Hash>::iterator it = M.Cells::find(p);
	switch (id) {
	case EX1 : // thin wall
	    break;
	case EX2 : // poisson mixed 2d
	    it->second->SetSubdomain(0);
	    break;
	case EX3 : // poisson mixed 3d
	    it->second->SetSubdomain(0);
	    break;
	}
    }
}

void AMBVP::SetBoundary (Mesh& M) {
    int id;
    ReadConfig(Settings, "EXAMPLE", id);
    for (bnd_face bf = M.bnd_faces(); bf != M.bnd_faces_end(); bf++) {
	Point p(bf());
	hash_map<Point,BoundaryFace,Hash>::iterator it = M.BoundaryFaces::find(p);
	switch (id) {
	case EX1 : {
	    if (pp->IsSource(bf)) {
		it->second.SetPart(1);
	    }
	    else if (bf()[2] == 0)
		it->second.SetPart(2);
	    else
		it->second.SetPart(3);
	    break;
	}
	case EX2 :
	    if (p[0] == 0 || p[1] == 0)
		it->second.SetPart(111); 
	    else
		it->second.SetPart(222);
	    break;
	case EX3 :
	    if (p[0] == 0 || p[1] == 0 || p[2] == 0)
		it->second.SetPart(DIRICHLET); 
	    else
		it->second.SetPart(NEUMANN);
	    break;
	}
    }
}

bool AMBVP::IsDirichlet (int id) {
    switch (example_id) {
    case EX1 :
	if (id == 1 || id == 2)
	    return true;
	return false;
	break;
    case EX2 :
	if (id == 111)
	    return true;
	return false;
	break;
    case EX3 : 
	return 1.0;
	break;
    }
}

double AMBVP::Coefficient (Point p) const { 
    switch (example_id) {
    case EX1 : 
	return 0.1;
	break;
    case EX2 : 
	return 1.0;
	break;
    case EX3 : 
	return 1.0;
	break;
    }
}

double AMBVP::Source (Point p) const { 
    switch (example_id) {
    case EX1 : 
	return 0;
	break;
    case EX2 : 
	return -2.0 * exp(p[0] + p[1]);
	break;
    case EX3 : 
	return -3.0 * exp(p[0] + p[1] + p[2]);
	break;
    }
    return 0.0;
}

double AMBVP::Dirichlet (Point p, int id) const { 
    switch (example_id) {
    case EX1 :
	if (id == 1)
	    return 1;
	else if (id == 2)
	    return 0;
	break;
    case EX2 :
	if (id == 111)
	    return exp(p[0] + p[1]); 
	break;
    case EX3 : 
	return exp(p[0] + p[1] + p[2]); 
	break;
    }
}

double AMBVP::Neumann (Point p, int id) const {
    switch (example_id) {
    case EX1 :
	if (id == 3)
	    return 0;
	break;
    case EX2 :
	if (id == 222)
	    return exp(p[0] + p[1]); 
	break;
    case EX3 :
	if (p[0] == 1)
	    return exp(p[0] + p[1] + p[2]);
	else if (p[1] == 1)
	    return exp(p[0] + p[1] + p[2]);
	else if (p[2] == 1)
	    return exp(p[0] + p[1] + p[2]); 
	break;
    }
}

double AMBVP::Solution (const Point& p) const { 
    switch (example_id) {
    case EX1 : 
	return 0;
	break;
    case EX2 : 
	return exp(p[0] + p[1]);
	break;
    case EX3 : 
	return exp(p[0] + p[1] + p[2]);
	break;
    }
}
